...
默认渲染器支持简洁的文本引用与可选图片缩略图。需要其他表现时，使用 `reference` slot 完整控制单项内容。
...
未声明的原生属性和监听器会继续传递到内部真实 `<textarea>`。计数器使用 SSR-safe ID 与输入框关联；受控值、IME composition、焦点和失焦行为与 `OTextarea` 保持一致。
