...
声明式示例依次展示单行反馈、多块插槽正文、可关闭消息和长内容。正文区域会在状态底片与关闭按钮之间自然换行，不需要为不同内容形态切换组件或增加标题 API。
...
Message 是当前组件库的特殊表面：浅色主题始终使用完全不透明的 `#fff`，深色主题始终使用 `#2d2d2d`。它不使用磨砂或透明背景，也不叠加外边框；层级只由实心表面和专用阴影表达。语义色只用于 `28px` 图标底片及其中的 `16px` 状态图标，外层表面不会随状态染色。
