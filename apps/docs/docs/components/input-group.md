...
Input Group 家族把文字、图标、按钮与现有输入控件组合成一圈完整边界。Root 是唯一边框拥有者，内部 OInput、OTextarea 与 OButton 继续保留原来的受控值、IME、autosize 和点击合同。
...
Root 也会通过 `:has([aria-invalid='true'])` 读取真实 control 错误状态。聚焦时只改变 border color，不添加 outline 或 glow。
...
- `OInputGroupInput` 完整使用 OInput Props，包含 modelValue、type、size、clearable、showPassword、disabled、readonly、invalid 与可访问名称。
- `OInputGroupTextarea` 完整使用 OTextarea Props，包含 modelValue、rows、autosize、maxlength、showCount、disabled、readonly 与 invalid。
- `OInputGroupButton` 使用 OButton Props；唯一默认差异是 size 从 md 调整为 sm。可显式覆盖 variant、size、tone、type、loading、iconOnly 与 disabled。
