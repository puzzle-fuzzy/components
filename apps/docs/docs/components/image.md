...
`OImage` 展示单张图片，并通过组件自身的 `Teleport` 遮罩层提供轻量全屏预览，不依赖 `ODialog` 或其他公开组件。预览图片约占视口 80%，点击遮罩或按 Esc 即可关闭。
...
- **遮罩**：深色半透明遮罩（`rgb(0 0 0 / 85%)`），聚焦图片内容
- **滚动**：预览打开期间锁定页面滚动，关闭后自动恢复
...
启用预览时，图片由原生 `button` 触发，通过 `previewAriaLabel` 提供独立的可访问名称。独立预览层使用 `role="dialog"` 与 `aria-modal="true"`；打开后焦点进入预览层，Tab 不会穿出，按 Esc 关闭后焦点返回预览入口。
